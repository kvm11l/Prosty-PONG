...
void pilka(){            // metoda tworzaca pilke
  fill(200,200,200);
  ellipse(x,y,w,h);      
}
...
void odbijanie(){        // odbijanie sie pilki od krawedzi
    if(x> width-w/2){
...
void ruchPaletka(){                      // ruchy paletkami
   if(upL){                              // lewa
...
void restrictPaletka(){                    // metoda zeby paletki nie uciekaly poza obszar
...
void odbiciaPaletka(){            // metoda ktora umozliwia odbicia paletkami
...
void wynik(){        // wyswietlanie wyniku
  fill(255);
  text(wynikL, 100,50);
...
void koncowyWynik(){          // ustawienie wyniku koncowego
 if(wynikL==koniec){
...
void keyPressed(){                // ruchy paletkami
  if(key == 'w' || key == 'W'){
   upL = true; 
  }  
  if(key == 's' || key == 'S'){
